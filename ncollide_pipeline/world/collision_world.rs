...
use broad_phase::{BroadPhase, DBVTBroadPhase, BroadPhasePairFilter, BroadPhasePairFilters, ProxyHandle};
use world::{CollisionObject, CollisionObjectHandle, CollisionObjectSlab, CollisionObjects,
            GeometricQueryType, CollisionGroups, CollisionGroupsPairFilter};
use events::{ContactEvents, ProximityEvents, ContactEvent};
...
impl<P: Point, M: Isometry<P>, T> CollisionWorld<P, M, T> {
    ...
    pub fn new(margin: P::Real) -> CollisionWorld<P, M, T> {
        ...
        let broad_phase     = Box::new(DBVTBroadPhase::<P, AABB<P>, CollisionObjectHandle>::new(margin));
    ...
    pub fn add(&mut self,
        ...
        let mut co = CollisionObject::new(CollisionObjectHandle::invalid(), ProxyHandle::invalid(), position, shape, collision_groups, query_type, data);
    ...
    pub fn remove(&mut self, handles: &[CollisionObjectHandle]) {
        {
            ...
            for handle in handles {
                let co = self.objects.get(*handle).expect("Removal: collision object not found.");
                proxy_handles.push(co.proxy_handle());
            }

            let nf = &mut self.narrow_phase;
            let objects = &mut self.objects;
            self.broad_phase.remove(&proxy_handles, &mut |b1, b2| nf.handle_removal(objects, *b1, *b2));
        }
        ...
        self.proximity_events.retain(|e| {
            objects.contains(e.collider1) && objects.contains(e.collider2)
        });
        self.contact_events.retain(|e| {
            match *e {
    ...
    pub fn set_position(&mut self, handle: CollisionObjectHandle, pos: M) {
        let co = self.objects.get_mut(handle).expect("Set position: collision object not found.");
    ...
    #[inline]
    pub fn collision_object(&self, handle: CollisionObjectHandle) -> Option<&CollisionObject<P, M, T>> {
        self.objects.get(handle)
    }

    /// Returns a mutable reference to the collision object identified by its handle.
    #[inline]
    pub fn collision_object_mut(&mut self, handle: CollisionObjectHandle) -> Option<&mut CollisionObject<P, M, T>> {
    ...
    pub fn interferences_with_point<'a>(&'a self, point: &'a P, groups: &'a CollisionGroups)
        ...
        // FIXME: avoid allocation.
        let mut handles = Vec::new();
        self.broad_phase.interferences_with_point(point, &mut handles);
    ...
    pub fn interferences_with_aabb<'a>(&'a self, aabb: &'a AABB<P>, groups: &'a CollisionGroups)
        ...
        // FIXME: avoid allocation.
        let mut handles = Vec::new();
        self.broad_phase.interferences_with_bounding_volume(aabb, &mut handles);
    ...
    fn filter_collision(filters: &BroadPhasePairFilters<P, M, T>,
                        ...
                        handle2: CollisionObjectHandle) -> bool {
