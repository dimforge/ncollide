...
use utils;
use shape::{SupportMap, AnnotatedPoint, AnnotatedMinkowskiSum, Reflection, Triangle};
...
impl<P: Point> Face<P> {
    pub fn new_with_proj(vertices: &[P], proj: P, pts: [usize; 3], adj: [usize; 3]) -> Self {
        ...
        if let Some(n) = P::ccw_face_normal(&[&vertices[pts[0]], &vertices[pts[1]], &vertices[pts[2]]]) {
            normal  = n;
            deleted = false;
        }
        else {
        ...
        Face { pts, proj, adj, normal, deleted, _marker }
    }
    ...
    pub fn new(vertices: &[P], pts: [usize; 3], adj: [usize; 3]) -> (Self, bool) {
        ...
        (Self::new_with_proj(vertices, proj.point, pts, adj), loc.is_on_face())
    }
    ...
    pub fn can_be_seen_by(&self, vertices: &[P], point: usize, opp_pt_id: usize) -> bool {
        ...
        na::dot(&(*pt - *p0), &self.normal) >= -gjk::eps_tol::<P::Real>() || utils::is_affinely_dependent_triangle3(p1, p2, pt)
    }
}
...
impl<P: Point> EPA3<P> {
    ...
    pub fn project_origin<M, S, G: ?Sized>(&mut self, m: &M, shape: &G, simplex: &S) -> P
        ...
        if simplex.dimension() == 0 {
            ...
            if proj_inside4 {
                let dist4 = na::dot(self.faces[3].normal.as_ref(), &self.vertices[3].coordinates());
    ...
    fn compute_silhouette(&mut self, point: usize, id: usize, opp_pt_id: usize) {
        if !self.faces[id].deleted {
            if !self.faces[id].can_be_seen_by(&self.vertices, point, opp_pt_id) {
                ...
                let adj_opp_pt_id1 = self.faces[adj1].next_ccw_pt_id(self.faces[id].pts[adj_pt_id1]);
                let adj_opp_pt_id2 = self.faces[adj2].next_ccw_pt_id(self.faces[id].pts[adj_pt_id2]);
    ...
    fn print_silhouette(&self) {
        ...
        for i in 0 .. self.silhouette.len() {
            ...
            if !face.deleted {
                print!("({}, {}) ", face.pts[(edge.opp_pt_id + 2) % 3], face.pts[(edge.opp_pt_id + 1) % 3]);
            }
        }
        println!("");
    }
...
pub fn closest_points<P, M, S, G1: ?Sized, G2: ?Sized>(epa:      &mut EPA3<AnnotatedPoint<P>>,
                                                           m1:       &M,
                                                           g1:       &G1,
                                                           m2:       &M,
                                                           g2:       &G2,
