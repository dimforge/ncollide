...
use query::{PointQuery, PointProjection, PointQueryWithLocation, TrianglePointLocation};
...
impl<N: Real> TetrahedronPointLocation<N> {
    ...
    pub fn same_feature_as(&self, other: &TetrahedronPointLocation<N>) -> bool {
        match (*self, *other) {
            (TetrahedronPointLocation::OnVertex(i),  TetrahedronPointLocation::OnVertex(j))  => i == j,
            (TetrahedronPointLocation::OnEdge(i, _), TetrahedronPointLocation::OnEdge(j, _)) => i == j,
            (TetrahedronPointLocation::OnFace(i, _), TetrahedronPointLocation::OnFace(j, _)) => i == j,
...
impl<P: Point> Tetrahedron<P> {
    ...
    #[inline]
    #[allow(dead_code)]
    fn project_point_with_location2<M: Isometry<P>>(&self, m: &M, pt: &P, solid: bool)
        -> (PointProjection<P>, TetrahedronPointLocation<P::Real>) {
            ...
            if smallest_id == 42 {
                return (PointProjection::new(true, *pt), TetrahedronPointLocation::OnSolid);
            }
            ...
            if let Some((ref proj, ref loc)) = projs[smallest_id] {
                let new_loc = match *loc {
                    TrianglePointLocation::OnVertex(i)   => TetrahedronPointLocation::OnVertex(vertices[smallest_id][i]),
                    TrianglePointLocation::OnEdge(i, uv) => TetrahedronPointLocation::OnEdge(edges[smallest_id][i], uv),
                    TrianglePointLocation::OnFace(uv)    => TetrahedronPointLocation::OnFace(smallest_id, uv),
...
impl<P: Point, M: Isometry<P>> PointQueryWithLocation<P, M> for Tetrahedron<P> {
    ...
    fn project_point_with_location(&self, m: &M, pt: &P, solid: bool)
        -> (PointProjection<P>, Self::Location) {
             ...
             */
            #[inline(always)]
            fn check_edge<P, M>(i: usize, m: &M, a: &P, b: &P,
                                nabc: &P::Vector, nabd: &P::Vector, ap: &P::Vector, ab: &P::Vector,
                                ap_ab: P::Real, /*ap_ac: P::Real, ap_ad: P::Real,*/
                                bp_ab: P::Real, /*bp_ac: P::Real, bp_ad: P::Real*/)
                                -> (P::Real, P::Real, Option<(PointProjection<P>, TetrahedronPointLocation<P::Real>)>)
                where P: Point,
                      M: Isometry<P> {
                let _0: P::Real = na::zero();
                ...
                if dabc >= _0 && dabd >= _0 && ap_ab >= _0 && ap_ab <= ab_ab {
                    ...
                    (dabc, dabd, Some((proj, TetrahedronPointLocation::OnEdge(i, bcoords))))
                }
                else {
                    (dabc, dabd, None)
                }
...
//            let bp_ac = bp_bc + bp_ab;
            ...
            let (dabc, dabd, res) = check_edge(0, m, self.a(), self.b(), &nabc, &nabd, &ap, &ab, ap_ab, /*ap_ac, ap_ad,*/ bp_ab/*, bp_ac, bp_ad*/);
            if let Some(res) = res {
                return res;
            }
...
//            let cp_ad = cp_cd + cp_ac;
            ...
            let (dacd, dacb, res) = check_edge(1, m, self.a(), self.c(), &nacd, &-nabc, &ap, &ac, ap_ac, /*ap_ad, ap_ab,*/ cp_ac/*, cp_ad, cp_ab*/);
            if let Some(res) = res {
                return res;
            }
...
//            let dp_ab = dp_ad - dp_bd;
            let (dadb, dadc, res) = check_edge(2, m, self.a(), self.d(), &-nabd, &-nacd, &ap, &ad, ap_ad, /*ap_ab, ap_ac,*/ dp_ad/*, dp_ab, dp_ac*/);
            if let Some(res) = res {
                return res;
            }
...
//            let cp_bd = cp_cd + cp_bc;
            ...
            // NOTE: nabc = nbcd
            let (dbca, dbcd, res) = check_edge(3, m, self.b(), self.c(), &nabc, &nbcd, &bp, &bc, bp_bc, /*-bp_ab, bp_bd,*/ cp_bc/*, -cp_ab, cp_bd*/);
            if let Some(res) = res {
                return res;
            }
...
//            let dp_bc = dp_bd - dp_cd;
            // NOTE: nbdc = -nbcd
            // NOTE: nbda = nabd
            let (dbdc, dbda, res) = check_edge(4, m, self.b(), self.d(), &-nbcd, &nabd, &bp, &bd, bp_bd, /*bp_bc, -bp_ab,*/ dp_bd/*, dp_bc, -dp_ab*/);
            if let Some(res) = res {
                return res;
            }
            ...
            // NOTE: ncda = nacd
            // NOTE: ncdb = nbcd
            let (dcda, dcdb, res) = check_edge(5, m, self.c(), self.d(), &nacd, &nbcd, &cp, &cd, cp_cd, /*-cp_ac, -cp_bc,*/ dp_cd/*, -dp_ac, -dp_bc*/);
            if let Some(res) = res {
                return res;
            }
             ...
             */
            #[inline(always)]
            fn check_face<P, M>(i: usize, a: &P, b: &P, c: &P, m: &M,
                                ap:   &P::Vector, bp: &P::Vector, cp: &P::Vector,
                                ab:   &P::Vector, ac: &P::Vector, ad: &P::Vector,
                                dabc:  P::Real, dbca:  P::Real, dacb: P::Real,
            ...
            // Face abc.
            if let Some(res) = check_face(0, self.a(), self.b(), self.c(), m,
                                          &ap, &bp, &cp, &ab, &ac, &ad,
                                          dabc, dbca, dacb,
                                          /*ap_ab, bp_ab, cp_ab,
                                          ap_ac, bp_ac, cp_ac*/) {
            ...
            // Face abd.
            if let Some(res) = check_face(1, self.a(), self.b(), self.d(), m,
                                          &ap, &bp, &dp, &ab, &ad, &ac,
                                          dadb, dabd, dbda,
                                          /*ap_ab, bp_ab, dp_ab,
                                          ap_ad, bp_ad, dp_ad*/) {
            ...
            // Face acd.
            if let Some(res) = check_face(2, self.a(), self.c(), self.d(), m,
                                          &ap, &cp, &dp, &ac, &ad, &ab,
                                          dacd, dcda, dadc,
                                          /*ap_ac, cp_ac, dp_ac,
                                          ap_ad, cp_ad, dp_ad*/) {
            ...
            // Face bcd.
            if let Some(res) = check_face(3, self.b(), self.c(), self.d(), m,
                                          &bp, &cp, &dp, &bc, &bd, &-ab,
                                          dbcd, dcdb, dbdc,
                                          /*bp_bc, cp_bc, dp_bc,
                                          bp_bd, cp_bd, dp_bd*/) {
