...
use convex_hull_utils::{support_point_id, indexed_support_point_id};
...
pub fn convex_hull2_idx<N: Real>(points: &[Point2<N>]) -> Vec<usize> {
    ...
    while i != segments.len() {
        ...
        let pt_id = indexed_support_point_id(&segments[i].normal,
                                             points,
...
fn get_initial_polyline<N: Real>(points: &[Point2<N>], undecidable: &mut Vec<usize>) -> Vec<SegmentFacet<N>> {
    let mut res = Vec::new();
...
fn attach_and_push_facets2<N: Real>(prev_facet:    usize,
                                    next_facet:    usize,
                                    point:         usize,
                                    points:        &[Point2<N>],
...
impl<N: Real> SegmentFacet<N> {
    pub fn new(p1: usize, p2: usize, prev: usize, next: usize, points: &[Point2<N>]) -> SegmentFacet<N> {
        let p1p2 = points[p2] - points[p1];
