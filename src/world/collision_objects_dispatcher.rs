...
use narrow_phase::{ContactSignal, ContactSignalHandler};
...
pub struct CollisionObjectsDispatcher<N, P, V, M, T> {
    ...
    pairs:            HashMap<Pair, AbstractCollisionDetector<N, P, V, M>, PairTWHash>
}

impl<N, P, V, M, T> CollisionObjectsDispatcher<N, P, V, M, T> {
    /// Creates a new `CollisionObjectsDispatcher`.
    pub fn new(shape_dispatcher: Rc<ShapeShapeDispatcher<N, P, V, M>>)
    ...
    pub fn contact_pairs(&self,
                         ...
                         f: |&T, &T, &AbstractCollisionDetector<N, P, V, M>| -> ()) {
    ...
    pub fn handle_proximity(&mut self,
        ...
        if started {
...

            {
                let co1 = &objects[*fk1];
                let co2 = &objects[*fk2];
                cd = self.shape_dispatcher.dispatch(&**co1.shape, &**co2.shape);
            }
