...
pub struct AABBSetsInterferencesCollector<'a, N: 'a + Real, T: 'a> {
    /// The transform from the local-space of the second bounding volume to the local space of the first.
    pub ls_m2: &'a Isometry<N>,
    /// The absolute value of the rotation matrix representing `ls_m2.rotation`.
    ///
    /// Equals to `ls_m2.to_rotation.matrix().matrix().abs()`.
...
impl<'a, N: Real, T: Clone> SimultaneousVisitor<T, AABB<N>> for AABBSetsInterferencesCollector<'a, N, T> {
    ...
    fn visit(&mut self, left_bv: &AABB<N>, left_data: Option<&T>, right_bv: &AABB<N>, right_data: Option<&T>) -> VisitStatus {
        let ls_right_bv = AABB::from_half_extents(self.ls_m2 * right_bv.center(), self.ls_m2_abs_rot * right_bv.half_extents());
