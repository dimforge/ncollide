    ...
    fn toi_with_ray(&self, m: &M, ray: &Ray<P>, solid: bool) -> Option<<P::Vect as Vect>::Scalar> {
        ...
        for i in 0usize .. na::dim::<P>() {
            if na::is_zero(&ls_ray.dir[i]) {
                if ls_ray.orig[i] < (*self.mins())[i] || ls_ray.orig[i] > (*self.maxs())[i] {
                    return None
                }
            }
            else {
                ...
                let mut inter_with_near_plane = ((*self.mins())[i] - ls_ray.orig[i]) * denom;
                let mut inter_with_far_plane  = ((*self.maxs())[i] - ls_ray.orig[i]) * denom;
...
fn do_toi_and_normal_and_uv_with_ray<M, P>(m: &M, aabb: &AABB<P>, ray: &Ray<P>, solid: bool)
    ...
    if na::dim::<P>() != 3 {
        ...
        ray_aabb(aabb, &ls_ray, solid).map(|(t, n, s)| {
            let pt    = ls_ray.orig + ls_ray.dir * t;
            ...
            if id == 1 {
                RayIntersection::new_with_uvs(t, gs_n, Some(Pnt2::new(dpt[1] / scale[1], dpt[2] / scale[2])))
            }
            else if id == 2 {
                RayIntersection::new_with_uvs(t, gs_n, Some(Pnt2::new(dpt[2] / scale[2], dpt[0] / scale[0])))
            }
            else {
                RayIntersection::new_with_uvs(t, gs_n, Some(Pnt2::new(dpt[0] / scale[0], dpt[1] / scale[1])))
            }
        })
    }
}
