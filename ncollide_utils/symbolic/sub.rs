...
use symbolic::{UnivariateFn, BivariateFn, SymAdd, SymMult, SymNeg};
use symbolic;
...
/// Symbolic representation of the subtraction of two functions.
#[inline]
pub fn sub<A, B>(a: A, b: B) -> SymSub<A, B> {
    SymSub { a: a, b: b }
}

impl<A: UnivariateFn<N, O>, B: UnivariateFn<N, O>, N: Copy, O: Sub<O, O>>
UnivariateFn<N, O> for SymSub<A, B> {
    #[inline]
    fn d0(&self, t: N) -> O {
...
impl<A: BivariateFn<N, O>, B: BivariateFn<N, O>, N: Copy, O: Sub<O, O> + Clone>
BivariateFn<N, O> for SymSub<A, B> {
    #[inline]
